...
import { TrendingUp, Users, Heart, Clock, ArrowUpRight, ArrowDownRight, BarChart3 } from 'lucide-react';
...
export default function AnalyticsPage() {
  ...
  // Calculations jo pool data par depend karti hain
  const stats = useMemo(() => {
    const totalPool = pool.length;
    const malePool = pool.filter(p => p.personal.gender === 'Male').length;
    const femalePool = pool.filter(p => p.personal.gender === 'Female').length;
    const avgIncome = pool.length > 0 ? Math.round(pool.reduce((s, p) => s + p.professional.income, 0) / pool.length / 100000) : 0;
  ...
  return (
    <div className="flex h-screen bg-slate-50 flex-col md:flex-row">
      {/* Assuming Sidebar handles its own mobile state (e.g., hidden or bottom nav on mobile) */}
      <Sidebar />
      
      <div className="flex-1 overflow-auto">
        {/* Added responsive padding (px-4 for mobile, md:px-8 for desktop) */}
        <div className="px-4 sm:px-6 md:px-8 py-6 md:py-7 max-w-[1100px] animate-fadeIn w-full mx-auto md:mx-0">
...
function SummaryCard({ icon: Icon, label, value, sub, color }: any) {
  ...
  return (
    <div className="tdc-card p-4 sm:p-5">
